...
const AGENT_SKILLS_PATH = path.join(__dirname, '../config/agent-skills.json');

function slug(name) {
    return path.basename(name, '.md').replace(/[^a-z0-9-]/gi, '-').toLowerCase();
}
...
metadata:
  author: reporting-tool
  source: ${AGENT_SKILLS_PATH}
---
...
function installSkill(agentType, promptPath) {
    let skills = {};
    try { skills = JSON.parse(fs.readFileSync(AGENT_SKILLS_PATH, 'utf8')); } catch {}
    ...
    if (!skills[agentType]) skills[agentType] = [];
    if (!skills[agentType].includes(fileName)) {
        skills[agentType].push(fileName);
    }
    fs.writeFileSync(AGENT_SKILLS_PATH, JSON.stringify(skills, null, 2));
}
